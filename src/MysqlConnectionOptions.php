...
final readonly class MysqlConnectionOptions
{
    ...
    public static function fromArray(array $options): self
    {
        ...
        if (!is_array($attributes)) {
            throw MysqlConnectionException::forInvalidOptions('MySQL auth PDO attributes must be an array.');
        }

        return new self(
            username: self::stringOption($options, self::DEFAULT_USERNAME, 'username', 'user'),
            password: self::stringOption($options, self::DEFAULT_PASSWORD, 'password', 'pass'),
    ...
    private function assertValid(): void
    {
        if (trim($this->username) === '') {
            throw MysqlConnectionException::forInvalidOptions('MySQL auth username must be a non-empty string.');
        }

        if (trim($this->database) === '') {
            throw MysqlConnectionException::forInvalidOptions('MySQL auth database name must be a non-empty string.');
        }

        if ($this->unixSocket === null && trim($this->host) === '') {
            throw MysqlConnectionException::forInvalidOptions('MySQL auth host must be a non-empty string.');
        }

        if ($this->port < 1 || $this->port > 65535) {
            throw MysqlConnectionException::forInvalidOptions('MySQL auth port must be between 1 and 65535.');
        }

        if (trim($this->charset) === '') {
            throw MysqlConnectionException::forInvalidOptions('MySQL auth charset must be a non-empty string.');
        }

        if ($this->unixSocket !== null && trim($this->unixSocket) === '') {
            throw MysqlConnectionException::forInvalidOptions('MySQL auth unix socket must be a non-empty string.');
        }

        if ($this->timeout !== null && $this->timeout < 1) {
            throw MysqlConnectionException::forInvalidOptions('MySQL auth timeout must be greater than zero.');
        }
    }
    ...
    private static function stringOption(array $options, string $default, string ...$keys): string
    {
        ...
        if (!is_string($value)) {
            throw MysqlConnectionException::forInvalidOptions(
                'MySQL auth option "' . $keys[0] . '" must be a string.',
            );
        }
    ...
    private static function intOption(array $options, int $default, string ...$keys): int
    {
        ...
        if (!is_int($value)) {
            throw MysqlConnectionException::forInvalidOptions(
                'MySQL auth option "' . $keys[0] . '" must be an integer.',
            );
        }
    ...
    private static function boolOption(array $options, bool $default, string ...$keys): bool
    {
        ...
        if (!is_bool($value)) {
            throw MysqlConnectionException::forInvalidOptions(
                'MySQL auth option "' . $keys[0] . '" must be a boolean.',
            );
        }
