...
final class MysqlStatementBinder
{
    /**
    ...
    public function bind(PDOStatement $statement, array $parameters, string $query = ''): void
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            if (!$statement->bindValue($parameter, $this->normalizeValue($value), $this->parameterType($value))) {
                throw MysqlQueryException::forBinding($parameter, $query);
            }
        }
    }
    ...
    private function namedParameter(string $key): string
    {
        ...
        if ($key === '' || $key === ':') {
            throw MysqlQueryException::forInvalidParameter($key, 'named parameters cannot be empty.');
        }
    ...
    {
        if ($key < 0) {
            throw MysqlQueryException::forInvalidParameter($key, 'positional parameters cannot be negative.');
        }
